...
public class BenchmarkSingleThreadJavolution5 {
	...
	@Benchmark
	public void FastListForeach(Blackhole bh) {
		for(double value : fastList) {
			bh.consume(value);
		}
	}
	@Benchmark
	...
	@Benchmark
	public void FastListForwardGetIndex(Blackhole bh) {
		for(int i = -1, s = fastList.size(); ++i < s;) {
			bh.consume(fastList.get(i));
		}
	}
	@Benchmark
	...
	@Benchmark
	public void FastListBackwardGetIndex(Blackhole bh) {
		for(int i = fastList.size(); --i >= 0;) {
			bh.consume(fastList.get(i));
		}
	}
	@Benchmark
	...
	public static void main(String[] args) throws RunnerException {
		Options opt = new OptionsBuilder().include(BenchmarkSingleThreadJavolution5.class.getSimpleName()).build();
		new Runner(opt).run();
	}
}